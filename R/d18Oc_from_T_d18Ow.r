#' Function to convert T data to d18O
#' 
#' Takes a matrix of T data (in degrees C) against time (in days), information
...
#' Kim and O'Neil, 1997 or Grossman and Ku, 1986). Converts the T data to d18O
#' data using the supplied empirical transfer function.
#' 
#' @param T Matrix with a time column (values in days) and an T column
#' (values in degrees C)
#' @param d18Ow Either a single value (constant d18Ow) or a vector of length
#' equal to the period in T data (365 days by default) containing information
...
#' Kim and O'Neil (1997).
#' @return A vector containing d18Oc values for each T value in \code{"T"}
...
#' d18O <- d18O_model(T, 0, "KimONeil97")
#' @export
d18Oc_from_T_d18Ow <- function(T, # Function that converts T values into d18O 
    ...
    if(!is.numeric(d18Ow)){
        d18Ow <- rep(0, length(T[,1])) # Set default d18Ow vector to constant at d18Ow = 0
    }else if(length(d18Ow) == 1){
        d18Ow <- rep(d18Ow, length(T[,1])) # Create constant d18Ow vector if only one number is given
    }else{
        d18Ow <- c(0, rep(d18Ow, length(T[,1])/length(d18Ow))) # If d18Ow is a vector with more than one value, multiply it to reach the same length of T (multiply by "years")
    }
    if(transfer_function == "KimONeil97"){
        d18Oc <- cbind(T[, 1], ((exp(((18.03 * 10 ^ 3) / (T[,2] + 273.15) - 32.42) / 1000) * (((d18Ow - 30.92) / 1.03092) / 1000 + 1)) - 1) * 1000) # Use Kim and O'Neil (1997) with conversion between VSMOW and VPDB by Brand et al. (2014) and Kim et al. (2015)
    }else if(transfer_function == "GrossmanKu86"){
        d18Oc <- cbind(T[, 1], (20.6 - T[,2]) / 4.34 + d18Ow - 0.2) # Use Grossmann and Ku (1986) modified by Dettmann et al. (1999)
    }else{
