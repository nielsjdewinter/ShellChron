...
#' }
#' testarray2 <- AgeCorr(testarray, 365, FALSE, FALSE) # Apply function on array
#' @export
AgeCorr <- function(resultarray, T_per, plot, agecorrection){
    # Check on glitches where consecutive windows are placed (almost) 1 year apart.
    # These are repaired by adding one T_per to the cumulative Day values of those windows in resultarray[, , 3]
    ...
    if(agecorrection == TRUE){
        agecorr <- rep(0, length(mean_window_age[, 2])) # Create vector to store corrections (in days) to the mean ages of modelling windows
        for(i in 1:(length(mean_window_age[,2]) - 1)){ # Loop through all windows
            if(mean_window_age[i + 1, 2] - mean_window_age[i, 2] > T_per / 2){ # If there is a large (> half a year) positive step between consecutive windows...
                agecorr[(i + 1):length(agecorr)] <- agecorr[(i + 1):length(agecorr)] - T_per # ...subtract one year from the subsequent windows
            }else if(mean_window_age[i + 1, 2] - mean_window_age[i, 2] < T_per / -2){ # If there is a large (> half a year) negative drop between consecutive windows...
                agecorr[(i + 1):length(agecorr)] <- agecorr[(i + 1):length(agecorr)] + T_per # ...add one year from the subsequent windows
            }
        }

        if(min(mean_window_age$Age + agecorr) > T_per){
            agecorr <- agecorr - 365 # Subtract whole number of years from correction if the smallest value is more than one year old
        }
        ...
        if(plot == TRUE){ # Plot updated result
            ageplot <- ggplot2::ggplot(mean_window_age, ggplot2::aes(window, Age)) + # Plot Window age to check for strange jumps in age
            ggplot2::geom_line() +
            ggplot2::geom_point() +
            ggplot2::ggtitle("Plot of average ages of modelling windows") +
            ggplot2::xlab("Window #") +
        ...
        resultarray[, 6:length(resultarray[1, , 1]), 3] <- resultarray[, 6:length(resultarray[1, , 1]), 3] + matrix(agecorr, nrow = length(resultarray[, 1, 1]), ncol = length(agecorr), byrow = TRUE) # Apply correction on the results of age modelling
    }else{
        resultarray[, 6:length(resultarray[1, , 1]), 3] <- resultarray[, 6:length(resultarray[1, , 1]), 3] - floor(min(mean_window_age$Age) / 365) * 365 # Correct age if all windows occur past year one
    }
    return(resultarray)
}