...
#' # Run function
#' result <- MCerr_proj(x, x_err, y, y_err, X, Y, 1000)
#' @export
MCerr_proj <- function(x, x_err, y, y_err, X, Y, MC){ # Function to propagate combined errors on x and y on the modelled X and Y values by means of local projection of y uncertainty on x and subsequent combination of uncertainties in X domain
